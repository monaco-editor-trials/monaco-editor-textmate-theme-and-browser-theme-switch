...
(async function () {
  ...
  // #region Auto Theme Switch

  const colorSchemeQuery = window.matchMedia('(prefers-color-scheme: dark)');

  const switchTheme = (): void => {
    const isDarkModeActive = colorSchemeQuery.matches;

    if (isDarkModeActive) {
      loadTheme(themes.find((theme: any) => theme.name === 'monokai'));
    } else {
      loadTheme(themes.find((theme: any) => theme.name === 'one-light'));
    }
  };

  colorSchemeQuery.addEventListener('change', () => switchTheme());
